...
class ModelSoftActorCritic:
    def __init__(self, observation_space, action_space, ac_kwargs, 
        ...
        self.dynam = dynam.MLPModel(observation_space.shape[0], action_space.shape[0])
    ...
    def plan_action(self, o):
        act = self.ac.act(torch.as_tensor(o, dtype=torch.float32).to(device), False)
        rew = self.dynam(o, act)
        for i in range(4):
            p_act = self.ac.act(torch.as_tensor(o, dtype=torch.float32).to(device), False)
            _, p_rew = self.dynam(o, p_act)
            if p_rew > rew:
                act = p_act
        return act
...
def main(env_fn, args, steps_per_epoch = 4000, epochs = 2500,
    ...
    msac = ModelSoftActorCritic(env.observation_space, env.action_space,
    ...
    def test_agent(num_test_episodes = 10):
        for j in range(num_test_episodes):
            ...
            while not(d or (ep_len == max_ep_len)):
                o, r, d, _ = test_env.step(msac.get_action(o, True))
                ep_ret += r
                ep_len += 1
    ...
    for t in range(total_steps):
        ...
        if t >= update_after and t % update_every == 0:
            ...
            for j in range(update_every):
                ...
                batch = replay_buffer.sample_batch(batch_size)
                msac.update_model_based(data=batch)
        ...
        if t >= 1000:
            batch = replay_buffer.sample_batch(batch_size)
            m_info = msac.update_model(data=batch)
